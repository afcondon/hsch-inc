...
(() => {
  ...
  // output/Hylograph.LibShell/index.js
  ...
  var screenshotLink = function(imgSrc) {
    return function(demoUrl) {
      return function(altText) {
        return a([href4(demoUrl), classes(["lib-screenshot-link"]), target5("_blank")])([img([src9(imgSrc), alt5(altText), classes(["lib-screenshot-link__img"])])]);
      };
    };
  };
  var para = function(text6) {
    return p([classes(["lib-elaboration__para"])])([text5(text6)]);
  };
  var navLink = function(label5) {
    return function(href5) {
      return a([href4(href5), classes(["lib-shell__nav-link"])])([text5(label5)]);
    };
  };
  var heroWithViz = function(config2) {
    return function(textContent2) {
      return function(vizContent) {
        return section([classes(["lib-hero-viz"])])([div2([classes(["lib-hero-viz__text"])])(append5([h2([classes(["lib-hero-viz__tagline"])])([text5(config2.tagline)]), div2([classes(["lib-hero-viz__install"])])([code_([text5("spago install " + config2.name)])])])(textContent2)), div2([classes(["lib-hero-viz__viz"])])([vizContent])]);
      };
    };
  };
  var header2 = function(config2) {
    return header([classes(["lib-shell__header"])])([div2([classes(["lib-shell__header-inner"])])([a([href4(config2.polyglotUrl), classes(["lib-shell__logo"])])([span3([classes(["lib-shell__logo-text"])])([text5("PSD3")])]), div2([classes(["lib-shell__title-group"])])([h1([classes(["lib-shell__title"])])([text5(config2.title)]), span3([classes(["lib-shell__version"])])([text5("v" + config2.version)])]), nav([classes(["lib-shell__nav"])])([navLink("Overview")("#"), navLink("API Docs")(config2.docsPath), navLink("GitHub")("https://github.com/" + config2.github)])])]);
  };
  var footer2 = function(config2) {
    return footer([classes(["lib-shell__footer"])])([div2([classes(["lib-shell__footer-inner"])])([span_([text5(config2.name + " is part of "), a([href4(config2.polyglotUrl)])([text5("Polyglot PureScript")])]), span_([text5(" \xB7 "), a([href4("https://github.com/" + config2.github)])([text5("Source on GitHub")])])])]);
  };
  var shell = function(config2) {
    return function(content3) {
      return div2([classes(["lib-shell"])])([header2(config2), main([classes(["lib-shell__main"])])(content3), footer2(config2)]);
    };
  };
  var elaboration = function(sections) {
    var renderSection = function(sect) {
      return div2([classes(["lib-elaboration__section"])])(append5([h3([classes(["lib-elaboration__heading"])])([text5(sect.heading)])])(sect.content));
    };
    return section([classes(["lib-elaboration"])])(map17(renderSection)(sections));
  };
  var codeExample = function(title4) {
    return function(code2) {
      return section([classes(["lib-example"])])([h3([classes(["lib-example__heading"])])([text5(title4)]), pre([classes(["lib-example__code"])])([code_([text5(code2)])])]);
    };
  };

  // output/Main/index.js
  var heroViz = /* @__PURE__ */ screenshotLink("demo.jpeg")("/anscombe/")("Anscombe's Quartet Demo");
  var heroText = [/* @__PURE__ */ p_([/* @__PURE__ */ text5("Audio interpreter for PSD3 - Data Sonification and Accessibility")])];
  var config = {
    name: "psd3-music",
    title: "Music",
  ...
  var codeSnippet = '-- This code works with both D3 and Music interpreters!\nsonifyData :: forall m sel. SelectionM sel m => Array Number -> m Unit\nsonifyData dataset = do\n  context <- select "audio"\n\n  tones <- context\n    # renderData Tone dataset "tone"\n        (Just \\d ->\n          [ time (\\i _ -> i * 0.5)        -- Evenly spaced in time\n          , pitch (\\_ d -> 200.0 + d * 10.0)  -- Map data to frequency\n          , duration (\\_ _ -> 0.3)        -- 300ms notes\n          , volume (\\_ d -> d / 100.0)    -- Map data to volume\n          , timbre (\\_ _ -> "sine")       -- Pure sine wave\n          ])\n        Nothing\n        Nothing\n\n  pure unit\n\n-- Interpret as visualization\nmain = do\n  runD3M $ sonifyData anscombeQuartet1\n\n-- Interpret as sonification\nmain = do\n  runMusicM $ sonifyData anscombeQuartet1';
  var render = function(v) {
    return shell(config)([heroWithViz(config)(heroText)(heroViz), elaboration([{
      heading: "Vision",
      content: [para("This package explores data sonification by implementing an audio interpreter for the PSD3 tagless DSL. The same PSD3 code that creates visual charts can be interpreted as sound, making data accessible through hearing rather than sight.")]
    }, {
      heading: "1. Accessibility",
      content: [para("Data visualization is inherently visual, excluding:"), para("Unsighted or visually impaired users"), para("Users with visual attention occupied (pilots, drivers, surgeons)"), para("Situations where visual displays are impractical"), para("Audio provides an alternative modality for data comprehension. Just as a scatter plot reveals patterns spatially, a sonification can reveal patterns temporally and spectrally.")]
    }, {
      heading: "2. Demonstrating Finally Tagless",
      content: [para("This package proves that PSD3's tagless architecture truly separates description from interpretation:"), para("Same PSD3 program"), para("Multiple interpreters: D3 (visual), English (text), WebAudio (sound)"), para("No changes to user code")]
    }, {
      heading: "3. Foundation for Music DSL",
      content: [para("While focused on sonification, this work explores patterns that could inform a future music composition DSL inspired by:"), para("Tidal Cycles' mini-notation"), para("Modular synthesis patching philosophy"), para("The data join pattern for separating temporal structure from musical content")]
    }, {
      heading: "Conceptual Mapping",
      content: [para("| D3/Visual Domain | Audio/Music Domain |"), para("|------------------|-------------------|"), para('| select "svg" | Create audio context |'), para('| selectAll "circle" | Create array of sound events |'), para("| Data join | Bind data to sonic parameters |"), para('| attr "cx" (x position) | Time offset (when to play) |'), para('| attr "cy" (y position) | Pitch (frequency in Hz) |'), para('| attr "r" (radius) | Duration or volume |'), para('| attr "fill" (color) | Timbre (waveform shape) |'), para("| Parent/child hierarchy | Sequential/parallel composition |"), para("| Enter/Update/Exit | Sound onset/sustain/release |")]
    }, {
      heading: "Anscombe's Quartet",
      content: [para("Anscombe's Quartet demonstrates why this matters. Four datasets with identical statistics:"), para("Same mean"), para("Same variance"), para("Same correlation"), para("But when visualized, they look completely different. And when sonified, they would sound completely different:"), para("Dataset 1: Linear progression (smooth ascending/descending melody)"), para("Dataset 2: Quadratic (curved melodic contour)"), para("Dataset 3: Linear with outlier (melody interrupted by jarring note)"), para("Dataset 4: Vertical line with outlier (repeated note with one exception)"), para("The patterns that vision reveals spatially, hearing reveals temporally and spectrally.")]
    }, {
      heading: "Use Cases",
      content: [para("1. Data Exploration - Listen to data distributions, outliers, trends"), para("2. Accessibility - Make charts and visualizations available to blind users"), para("3. Monitoring - Audio dashboards for system metrics (like Geiger counters for data)"), para("4. Multimodal Analysis - Use both vision and hearing simultaneously for richer understanding"), para("5. Education - Teach data literacy through multiple sensory modes")]
    }, {
      heading: "Architecture",
      content: [para("The MusicSelection_ type represents scheduled audio events rather than DOM elements, but the operations are the same: select, join data, set attributes, handle enter/update/exit.")]
    }, {
      heading: "Relationship to Music Composition DSL (Future)",
      content: [para("This sonification work is distinct from but related to a potential music composition DSL. Key differences:"), para("psd3-music (this package):"), para("Data \u2192 Sound (sonification)"), para("Input: Arbitrary data arrays"), para("Output: Audio representation of data patterns"), para("Goal: Accessibility and data comprehension"), para("Future Music DSL:"), para("Musical ideas \u2192 Sound (composition)"), para("Input: Musical structures (mini-notation, chord progressions, melodies)"), para("Output: Music"), para("Goal: Creative music making and generative composition"), para("Shared patterns:"), para("Typed data joins (structure + content)"), para("Multiple interpreters (audio, notation, visualization, English)"), para("Finally tagless architecture"), para('Separation of "what/when" (temporal structure) from "how" (sonic content)'), para("Both benefit from exploring how the join pattern works in the temporal/auditory domain.")]
